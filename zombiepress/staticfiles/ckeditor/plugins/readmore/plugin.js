// Based on http://wordpress.org/plugins/ckeditor-for-wordpress/
CKEDITOR.plugins.add('readmore',
{
    onLoad: function()
    {
        CKEDITOR.addCss(
            'img.zombiepress-readmore' +
            '{' +
                'background-image: url(' + CKEDITOR.getUrl( this.path + 'images/more_el.gif' ) + ');' +
    ...
    init: function(editor)
    {
        editor.addCommand('readMoreCmd', {
            ...
            exec: function()
            {
                ...
                var el = editor.document.$.querySelector('img.zombiepress-readmore');
                if (el) {
                    if (confirm('The document already contains a read more. Do you want to proceed by removing it first?'))
                    {
                        el.remove();
                    }
                }
        ...
        function insertComment( text )
        {
            ...
            var fakeElement = editor.createFakeElement( new CKEDITOR.dom.comment( text ), 'zombiepress-' + text, 'hr' );
        ...
        editor.dataProcessor.dataFilter.addRules(
            {
                comment : function( value )
                {
                    ...
                    if ( value == 'readmore' )
                        return editor.createFakeParserElement(new CKEDITOR.htmlParser.comment(value), 'zombiepress_' + value, 'hr' );
